    ...
    public static class VHierarchy
    {
...
#if UNITY_6000_3_OR_NEWER
            Object getObject(ObjectID id) => EditorUtility.EntityIdToObject(id);
            ObjectID getSceneId(Scene scene) => scene.handle.GetFieldValue<ObjectID>("m_Value");
#else
        ...
        static void GameObjectRowGUI(GameObject go, Rect rowRect)
        {
            ...
            void hydrateData()
            {
                void scene()
                {
                    ...
                    {
                        iids = new ObjectID[gids.Count];

#if UNITY_6000_3_OR_NEWER
                        GlobalObjectId.GlobalObjectIdentifiersToEntityIdsSlow(gids.ToArray(), iids);
#else
                ...
                void go_()
                {
                    sceneData.goDatasByInstanceId.TryGetValue(go.GetObjectID(), out goData);
                }
            ...
            void ensureGoDataExists()
            {
                ...
                sceneData.goDatasByGlobalId[GlobalObjectId.GetGlobalObjectIdSlow(go.GetObjectID()).ToString()] = goData;
                sceneData.goDatasByInstanceId[go.GetObjectID()] = goData;
        ...
        static void SceneRowGUI(Scene scene, Rect rowRect)
        {
            ...
            void collapseEverything()
            {
                ...
                foreach (var iid in expandedIds)
                    if (_EditorUtility_ObjectIDToObject(iid) is GameObject expandedGo && expandedGo.scene == scene)
                ...
                expandQueue_toAnimate = expandedRoots.Select(r => new ExpandQueueEntry(r.GetObjectID(), false))
        ...
        static void CheckShortcuts()
        {
            ...
            void toggleExpanded()
            {
                ...
                SetExpandedWithAnimation(hoveredGo.GetObjectID(), !expandedIds.Contains(hoveredGo.GetObjectID()));
                EditorApplication.RepaintHierarchyWindow();
            ...
            void collapseEverythingElse()
            {
                ...
                foreach (var iid in expandedIds.ToList())
                    if (_EditorUtility_ObjectIDToObject(iid) is GameObject expandedGo && !parents.Contains(expandedGo) && expandedGo != hoveredGo)
                        toCollapse.Add(expandedGo);


                expandQueue_toAnimate = toCollapse.Select(r => new ExpandQueueEntry(r.GetObjectID(), false))
                                                         .Append(new ExpandQueueEntry(hoveredGo.GetObjectID(), true))
                                                         .OrderBy(r => VisibleRowIndex(r.instanceId)).ToList();
        ...
        static void UpdateExpandQueue()
        {
            ...
            if (!expandQueue_toAnimate.Any())
            {
                ...
                foreach (var r in expandQueue_toCollapseAfterAnimation)
                    SetExpanded(r.GetObjectID(), false);
        ...
        struct ExpandQueueEntry { public ObjectID instanceId; public bool expand; public ExpandQueueEntry(ObjectID instanceId, bool expand) { this.instanceId = instanceId; this.expand = expand; } }

        static void UpdateExpandedIdsList()
        {
            expandedIds = hierarchyWindow?.GetFieldValue("m_SceneHierarchy")?.GetFieldValue<TreeViewState>("m_TreeViewState").expandedIDs ?? new List<ObjectID>();

            EditorApplication.delayCall -= UpdateExpandedIdsList;
            EditorApplication.delayCall += UpdateExpandedIdsList;

        }
        static List<ObjectID> expandedIds;

        static void SetExpandedWithAnimation(ObjectID instanceId, bool expanded) => hierarchyWindow.GetFieldValue("m_SceneHierarchy").GetFieldValue("m_TreeView").InvokeMethod("ChangeFoldingForSingleItem", instanceId, expanded);
        static void SetExpanded(ObjectID instanceId, bool expanded) => hierarchyWindow.InvokeMethod("SetExpanded", instanceId, expanded);
        static bool IsExpanded(GameObject go) => expandedIds.Contains(go.GetObjectID());
        ...
        static int VisibleRowIndex(ObjectID instanceId) => hierarchyWindow.GetFieldValue("m_SceneHierarchy").GetFieldValue("m_TreeView").GetPropertyValue("data").InvokeMethod<int>("GetRow", instanceId);
